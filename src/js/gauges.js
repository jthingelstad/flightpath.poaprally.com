...
(function () {
  if (typeof CLAIMS === "undefined" || typeof LEADERBOARDS === "undefined") return;
  ...
  const COLORS = {
    ...
    blue: "#6db3f8",
    cream: "#f0e6d3",
    month: ["#8076FA", "#9289FF", "#7168DE", "#6db3f8"],
    day: ["#F87588", "#9289FF", "#8076FA", "#7168DE", "#6db3f8", "#0FCEAD", "#F87588"],
    region: { "North America": "#8076FA", "Europe": "#9289FF", "Asia": "#F87588", "South America": "#0FCEAD", "Africa": "#6db3f8", "Oceania": "#f0e6d3" },
    default: ["#8076FA", "#9289FF", "#F87588", "#0FCEAD", "#6db3f8", "#f0e6d3"],
  };
  ...
  function buildAreaChart(data, container, opts) {
    ...
    points.forEach(function (p) {
      svg += '<circle cx="' + p.x + '" cy="' + p.y + '" r="3" fill="' + color + '" stroke="#3A3A3A" stroke-width="1.5"/>';
    });
    svg += '</svg>';
  ...
  function buildBarChart(data, container, opts) {
    opts = opts || {};
    const maxVal = Math.max.apply(null, data.map(function (d) { return d.value; }).concat([1]));
    const colors = opts.colors || COLORS.default;
  ...
  buildAreaChart(data, document.getElementById("gauge-cumulative"), {
    color: COLORS.primary, gradientId: "grad-cum",
  ...
  }), document.getElementById("gauge-by-month"), { colors: COLORS.month });
  ...
  }), document.getElementById("gauge-by-day"), { colors: COLORS.day });

  // By region
  buildBarChart(LEADERBOARDS.regions.map(function (r) {
    return { label: r.continent, value: r.claims, color: COLORS.region[r.continent] || COLORS.primary };
  ...
  }), document.getElementById("gauge-top-airports"), { colors: [COLORS.primary] });
